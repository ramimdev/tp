...
(function(){
  const menuBtn = document.getElementById('menuBtnPro');
  const drawer = document.getElementById('drawerMenuPro');
  const overlay = document.getElementById('overlayPro');
  const closeBtn = document.getElementById('closeMenuPro');
  ...
  function closeDrawer(){
    ...
    document.querySelectorAll('.submenuPro.open').forEach(s=>{
      s.classList.remove('open');
      s.setAttribute('aria-hidden','true');
      const p = s.parentElement.querySelector('.plusPro');
      if(p){ p.textContent='+'; p.classList.remove('active'); }
      const parentLink = s.parentElement.querySelector('a[aria-expanded]');
  ...
  document.querySelectorAll('.plusPro').forEach(btn=>{
    btn.addEventListener('click', function(e){
      e.stopPropagation();
      const li = btn.closest('li');
      const submenu = li.querySelector('.submenuPro');
      if(!submenu) return;
      ...
      document.querySelectorAll('.submenuPro.open').forEach(s=>{
        if(s!==submenu){
          s.classList.remove('open');
          s.setAttribute('aria-hidden','true');
          const otherBtn = s.parentElement.querySelector('.plusPro');
          if(otherBtn){ otherBtn.textContent='+'; otherBtn.classList.remove('active'); }
          const otherLink = s.parentElement.querySelector('a[aria-expanded]');
  ...
  document.querySelectorAll('.drawerMenuPro > ul > li > a[aria-haspopup]').forEach(link=>{
    link.addEventListener('click', function(e){
      e.preventDefault();
      const li = this.parentElement;
      const btn = li.querySelector('.plusPro');
      if(btn) btn.click();
    });
  });
